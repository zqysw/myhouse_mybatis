	...
	<body>
		<DIV id="header" class="wrap">
			...
			<DIV class="search">
				<LABEL class="ui-green searchs">
					<a href="publicprepare.action" title="">发布房屋信息</a>
				</LABEL>
				<LABEL class="ui-green searchs">
					...
					<c:forEach items="${requestScope.houselist}" var="house">
					 <TR>
							...
							<TD class="house-type">
									...
									<a href = "editprepare.action?id=${house.id }" >修改</a>
							
							</TD>
							<TD class="house-price">
							
									<a href = "del.action?id=${house.id }" onclick = "return del();">删除</a>	
							</TD>
						</TR>
				</c:forEach>		
					</TBODY>
		...
		<DIV class="pager">
				...
				<LI class="current">
					...
					<LI>
						<A href="list.action?page=${requestScope.pageindex-1 }">上一页</A>
					</LI>
				</c:if>
					<c:if test="${requestScope.pageindex<requestScope.pagecount}">
					
					<LI>
						<A href="list.action?page=${requestScope.pageindex+1 }">下一页</A>
					</LI>
					<LI>
						<A href="list.action?page=${requestScope.pagecount }">末页</A>
					</LI>
