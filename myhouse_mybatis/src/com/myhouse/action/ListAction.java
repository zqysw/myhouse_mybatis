...
import com.myhouse.myservicetest.DistrictService;
import com.myhouse.myservicetest.HouseInfoService;
import com.myhouse.myservicetest.HouseTypeService;
import com.myhouse.myservicetest.StreetService;
...
public class ListAction extends ActionSupport implements Preparable,SessionAware,RequestAware{
	...
	public String list(){
		...
		SelByName selbyname = new SelByName((pageindex-1)*Datechange.PAGESIZE, Datechange.PAGESIZE, name);
		List<HouseInfo> list = service.selHousesByName(selbyname);
	...
	public String edit(){
		...
		HouseInfo house = new HouseInfo(getHouse().getId(), getHouse().getTitle(), htype.getId(),getHouse().getFloorage(),
				getHouse().getPrice(), getHouse().getHousedate(), district.getId(), street.getId(), getHouse().getPhone(),
