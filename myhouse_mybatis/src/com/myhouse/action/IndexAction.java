...
import com.myhouse.myservicetest.HouseInfoService;
...
public class IndexAction extends ActionSupport implements SessionAware,RequestAware,CookiesAware{
	...
	public String search(){
		...
		if ("".equals(getTitle()) && "".equals(getPage()) && "".equals(getStreet_id())
			...
			SelCountByAll selcountbyall = new SelCountByAll(low_price, high_price,getTitle(), getType_id(), getStreet_id(), low_floorage,  high_floorage);
			int pagecount = service.getPageCountByAll(Datechange.PAGESIZE,selcountbyall);
			if (pagecount == 0) {
				pageindex = pagecount;
				request.put("message", "没有搜索到相关内容");
			}
			if(pagecount!=0){
				SearchAll searchall = new SearchAll((pageindex-1)*Datechange.PAGESIZE, Datechange.PAGESIZE, low_price, high_price, getTitle(), getType_id(), getStreet_id(), low_floorage, high_floorage);
				List<HouseInfo> list = service.search(searchall);
				for (HouseInfo house : list) {
					String dist = service.selDistrictTypeById(house
							.getDistrict_id());
					String str = service.selStreetTypeById(house.getStreet_id());
